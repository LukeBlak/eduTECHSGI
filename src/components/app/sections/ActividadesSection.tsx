...
export function ActividadesSection() {
  ...
  // que se borrarán + miembros afectados) para mostrarlo en la confirmación.
  async function handleRequestDelete(a: Activity) {
    setDeleteTarget(a);
    setDeleteImpact(null);
    setDeleteImpactLoading(true);
    try {
      const impact = await activitiesApi.deleteImpact(a.id);
      setDeleteImpact({
  ...
  async function handleDelete(a: Activity) {
    try {
      ...
      toast.success(
        count > 0
          ? `Actividad eliminada · ${count} hora(s) social(es) borradas`
          : "Actividad eliminada",
      );
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {filtered.map((a) => (
            <ActivityCard
              ...
              onDelete={(act) => handleRequestDelete(act)}
        ...
        <AlertDialogContent className="max-w-lg">
          ...
          {deleteImpactLoading ? (
            ...
            <div className="rounded-md border border-amber-300 bg-amber-50 dark:border-amber-900/50 dark:bg-amber-950/40 p-3 space-y-3">
              ...
              <div className="space-y-1.5">
                <p className="text-xs font-semibold uppercase tracking-wide text-amber-700 dark:text-amber-400">
                  Miembros afectados ({deleteImpact.affectedMembers.length}):
                </p>
            ...
            <div className="flex items-center gap-2 rounded-md border border-emerald-200 bg-emerald-50 dark:border-emerald-900/40 dark:bg-emerald-950/30 p-3 text-sm text-emerald-700 dark:text-emerald-300">
              ...
              Esta actividad no tiene horas sociales asignadas. La eliminación
