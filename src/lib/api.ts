...
export interface FirebaseConfigClient {
  projectId: string;
  clientEmail: string;
  databaseUrl: string;
  enabled: boolean;
...
export const firebaseApi = {
  getConfig: () => fetchApi<FirebaseConfigClient>("/firebase/config"),
  saveConfig: (config: Partial<FirebaseConfigClient> & { privateKey?: string }) =>
    fetchApi<{ success: boolean; message: string; config: FirebaseConfigClient }>("/firebase/config", {
      method: "PUT",
      body: JSON.stringify(config),
    }),
  status: () => fetchApi<FirebaseStatus>("/firebase/status"),
  test: () => fetchApi<{ success: boolean; message: string; projectId?: string }>("/firebase/test", {
    method: "POST",
  }),
  sync: () => fetchApi<SyncResult>("/firebase/sync", { method: "POST" }),
  push: () => fetchApi<SyncResult>("/firebase/push", { method: "POST" }),
  loadMock: () =>
    fetchApi<{ success: boolean; message: string; result: SyncResult }>("/firebase/mock", {
      method: "POST",
      body: JSON.stringify({}),
    }),
};
